...
export class Entity {
  ...
  tweenManager: TweenManager | null;
  poolManager: ObjectPoolManager | null = null;
  ...
  private addIdleAnimation() {
    ...
    this.idleDuration = 800 + Math.random() * 400;

    // 只有 sprite 上下浮动，影子保持不动
  ...
  protected updateVisuals(inViewport: boolean = true) {
    if (this.sprite) {
      this.sprite.setVisible(inViewport);
    }
  }
  ...
  protected showDamageEffect(isCrit: boolean = false) {
    ...
    if (this.body && this.poolManager) {
      const particlePool = this.poolManager.getParticlePool();
      const flash = particlePool.acquireParticle(this.x, this.y, 25, 0xff0000, 0.5);
      flash.setDepth(90);
      this.activeParticles.add(flash);
      
      const flashObj = flash.getGameObject();
      if (flashObj) {
        flashObj.setScale(1, 1);
        this.scene.tweens.add({
          targets: flashObj,
          alpha: 0,
          scaleX: 1.5,
          ...
          duration: 150,
          onComplete: () => {
            this.activeParticles.delete(flash);
            particlePool.releaseParticle(flash);
          },
        });
      }
    } else if (this.body) {
  ...
  private releaseAllPoolObjects() {
    if (!this.poolManager) return;

    const particlePool = this.poolManager.getParticlePool();
    const textPool = this.poolManager.getTextPool();
    const effectPool = this.poolManager.getEffectPool();

    this.activeParticles.forEach(particle => {
      const obj = particle.getGameObject();
      if (obj) {
    ...
    this.activeTexts.forEach(text => {
      const obj = text.getGameObject();
      if (obj) {
        this.scene.tweens.killTweensOf(obj);
      }
      textPool.releaseText(text);
    });
    this.activeTexts.clear();

    this.activeEffects.forEach(effect => {
      const obj = effect.getGameObject();
      if (obj) {
        this.scene.tweens.killTweensOf(obj);
      }
  ...
  private createDeathEffect() {
    const particleCount = Math.min(8, 15);
    ...
    if (this.poolManager) {
      ...
      const particles: PoolableParticle[] = [];
      const particleTargets: { x: number; y: number; alpha: number; scale: number }[] = [];
      ...
      for (let i = 0; i < particleCount; i++) {
        ...
        const particle = particlePool.acquireParticle(this.x, this.y, size, color, 1);
      ...
      particles.forEach((particle, index) => {
        const particleObj = particle.getGameObject();
        if (particleObj) {
          this.scene.tweens.add({
            targets: particleObj,
            x: particleTargets[index].x,
            y: particleTargets[index].y,
            alpha: 0,
            scaleX: 0.2,
            scaleY: 0.2,
      ...
      const shockwave = effectPool.acquireShockwave(this.x, this.y, 10, 0xff4444, 0.8, 3);
      shockwave.setDepth(94);
      this.activeEffects.add(shockwave);

      const shockwaveObj = shockwave.getGameObject();
      if (shockwaveObj) {
        this.scene.tweens.add({
          targets: shockwaveObj,
          scaleX: 4,
          scaleY: 4,
          alpha: 0,
          duration: 350,
          ...
          onComplete: () => {
            ...
            effectPool.releaseEffect(shockwave);
          },
        });
      }

      const flash = particlePool.acquireParticle(this.x, this.y, 25, 0xffffff, 0.7);
      flash.setDepth(96);
      this.activeParticles.add(flash);
      ...
      this.scene.tweens.add({
        targets: particles,
        x: (target: Phaser.GameObjects.Arc) => particleTargets[particles.indexOf(target)].x,
        y: (target: Phaser.GameObjects.Arc) => particleTargets[particles.indexOf(target)].y,
        alpha: 0,
        scaleX: 0.2,
        scaleY: 0.2,
        duration: 400 + Math.random() * 200,
  ...
  showAttackEffect(targetX?: number, targetY?: number, effectColor = 0xFFFFFF, isHeavy: boolean = false) {
    ...
    if (this.poolManager) {
      ...
      const effect = particlePool.acquireParticle(tx, ty, 10 * impactScale, effectColor, 0.9);
      effect.setDepth(100);
      this.activeParticles.add(effect);

      const effectObj = effect.getGameObject();
      if (effectObj) {
        this.scene.tweens.add({
          targets: effectObj,
          alpha: 0,
          ...
          ease: 'Power2',
          onComplete: () => {
            this.activeParticles.delete(effect);
            particlePool.releaseParticle(effect);
          },
        });
      }

      const slashCount = isHeavy ? 3 : 1;
      const slashes: PoolableEffect[] = [];
      ...
      for (let i = 0; i < slashCount; i++) {
        ...
        const slash = effectPool.acquireSlash(tx, ty + offset, angle1, length, lineWidth, effectColor, 0.9);
        slash.setDepth(100);
        this.activeEffects.add(slash);
        slashes.push(slash);

        const slashObj = slash.getGameObject();
        if (slashObj) {
          this.scene.tweens.add({
            targets: slashObj,
            alpha: 0,
            ...
            onComplete: () => {
              this.activeEffects.delete(slash);
              effectPool.releaseEffect(slash);
            },
          });
        }
      }
      ...
      const particles: PoolableParticle[] = [];
      const particleTargets: { x: number; y: number }[] = [];

      for (let i = 0; i < particleCount; i++) {
        const angle = (Math.PI * 2 * i) / particleCount;
        const speed = 20 + Math.random() * 30;
        const particle = particlePool.acquireParticle(tx, ty, 2 + Math.random() * 3, effectColor, 0.8);
      ...
      const slashCount = isHeavy ? 3 : 1;
      const slashes: Phaser.GameObjects.Graphics[] = [];
  ...
  showDamageNumber(damage: number, isCrit = false) {
    ...
    if (this.poolManager) {
      const textPool = this.poolManager.getTextPool();
      const text = textPool.acquireText(this.x, this.y - 30, `${Math.floor(damage)}`, {
        fontSize: size,
        color: color,
        fontFamily: 'serif',
        stroke: '#000000',
      ...
      const textObj = text.getGameObject();
      if (textObj) {
        this.scene.tweens.add({
          targets: textObj,
          y: this.y - 60,
  ...
  destroy() {
    ...
    if (this.tweenManager) {
      this.tweenManager.unregisterEntity(this.id);
    }
    this.sprite?.destroy();
  }
}
